...
export default async function DashboardPage() {
  ...
  const { recentLeads, recentQuotations, recentInvoices } = await getRecentActivity()
  ...
  const shootsThisWeek = [...allLeads, ...recentQuotations].filter(item => {
  ...
  const pipelineStages = [
    ...
    { stage: "Won", count: allLeads.filter(l => l.status === "Won").length, color: "#DDEFCB" },
  ]

  const followUpsDueToday = allLeads.filter(l => l.follow_up_date === todayStr)
  const shootsToday = [...allLeads, ...recentQuotations].filter(item => item.event_date === todayStr)
        ...
        >
          ...
          <div className="space-y-2">
            {followUpsDueToday.map(lead => (
              <div key={lead.id} className="flex items-center justify-between rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--app-bg)", border: "1px solid var(--border-color)" }}>
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <Clock className="w-4 h-4 flex-shrink-0" style={{ color: "#0E4F63" }} />
                  <div className="min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: "var(--text-primary)" }}>Follow-up: {lead.client_name}</p>
                    <p className="text-xs" style={{ color: "var(--text-secondary)" }}>{lead.project_type}</p>
                  </div>
                </div>
            ...
            <div className="space-y-1">
              {recentLeads.map(lead => (
                <Link
                  key={lead.id}
                  href={`/leads/${lead.id}`}
                  className="flex items-center justify-between px-3 py-2.5 rounded-2xl transition-colors hover:bg-black/5 dark:hover:bg-white/5"
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium truncate" style={{ color: "var(--text-primary)" }}>{lead.client_name}</p>
                    <p className="text-xs mt-0.5" style={{ color: "var(--text-secondary)" }}>{lead.project_type || "—"} · {formatDateShort(lead.created_at)}</p>
                  </div>
