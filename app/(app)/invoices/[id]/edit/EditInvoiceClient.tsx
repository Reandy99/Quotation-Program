...
type InvoiceStatusOption = "Draft" | "Unpaid" | "Paid"

const STATUSES: InvoiceStatusOption[] = ["Draft", "Unpaid", "Paid"]

function toStatusOption(status: InvoiceStatus): InvoiceStatusOption {
  if (status === "Paid") return "Paid"
  if (status === "Draft") return "Draft"
  return "Unpaid"
}

function toInvoiceStatus(option: InvoiceStatusOption, currentPaidAmount: number): InvoiceStatus {
  if (option === "Paid") return "Paid"
  if (option === "Draft") return "Draft"
  return currentPaidAmount > 0 ? "Partial" : "Sent"
}
...
export default function EditInvoiceClient({ invoice }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    notes: invoice.notes ?? "",
    status: toStatusOption(invoice.status),
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await updateInvoice(invoice.id, {
        ...
        status: toInvoiceStatus(form.status, invoice.paid_amount),
      })
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <select
                  value={form.status}
                  onChange={e => setForm(f => ({ ...f, status: e.target.value as InvoiceStatusOption }))}
