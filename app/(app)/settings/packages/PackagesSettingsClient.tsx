...
import { PACKAGES_KEY } from "@/lib/settings/storage"
import { updatePackagesSettings } from "../actions"
...
export default function PackagesSettingsClient({ initialPackages }: Props) {
  ...
  async function savePackages(updated: ServicePackage[]) {
    const result = await updatePackagesSettings(updated)
    ...
    toast({
      ...
      description: "Package settings updated successfully.",
    })
    return true
  }
  ...
  async function handleAddPackage() {
    ...
    const success = await savePackages([...packages, newPkg])
    if (!success) return
  ...
  async function handleUpdatePackage() {
    ...
    const success = await savePackages(packages.map(p => p.id === editingId ? { ...p, ...formData } : p))
    if (!success) return
    setEditingId(null)
    setFormData({ name: "", description: "", items: [] })
  }
  ...
  async function handleDeletePackage(id: string) {
    ...
    await savePackages(packages.filter(p => p.id !== id))
  }
